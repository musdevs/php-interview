# Пользовательский интерфейс в Битрикс24

## Bitrix24 UI for REST API

[Bitrix24 UI docs](https://bitrix24.github.io/b24ui/)
[Bitrix24 UI github](https://github.com/bitrix24/b24ui)

[Системные компоненты main.ui.grid, main.ui.filter, main.user.selector и main.field.*](https://dev.1c-bitrix.ru/api_d7/bitrix/main/systemcomponents/index.php)
[Отрисовка поля UserField через новое api](https://dev.1c-bitrix.ru/api_d7/bitrix/main/userfield/fieldrendering.php)

## [Bitrix24: Книга разработчика](https://bx24devbook.website.yandexcloud.net/)

[Тулбар](https://bx24devbook.website.yandexcloud.net/Razrabotka/UI/Tulbar/Osnovnoe.html)
[Фильтр bitrix:main.ui.filter](https://bx24devbook.website.yandexcloud.net/Razrabotka/UI/Filtr/O_module.html)
[Грид bitrix:main.ui.grid](https://bx24devbook.website.yandexcloud.net/Razrabotka/UI/Tablicy/Osnovnoe.html)

[Устаревшее - Гриды и фильтры в Битриксе](https://prominado.ru/blog/gridy-i-filtry-v-bitrix/)


### [Cкрыть определенное поле из профиля пользователя Битрикс24](https://www.pavelkvashin.ru/blog/ckryt-opredelennoe-pole-iz-profilya-polzovatelya-bitriks24/)

Для решения задачи понадобится событие BX.UI.EntityEditor:onInit. Пример скрытия поля Email из профиля сотрудника. Условие по которому отработает скрытие поля зависит от вашей задачи.

```js
 BX.ready(function() {

   BX.addCustomEvent('BX.UI.EntityEditor:onInit', (editor, fields) => {
            if (editor)
           {
               const control = editor.getControlByIdRecursive('EMAIL');
               if (control)
               {
                   control.getParent().removeChild(control, {
                       enableSaving: false
                   });
               }
           }
       });
});
```
