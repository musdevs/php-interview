...
или так (ключ +L1 показывает файлы, у которых количество ссылок меньше 1, то есть они уже удалены, но всё ещё используются процессом):
...
Если перезапуск сервиса нежелателен, можно очистить файл через файловый дескриптор процесса в /proc.
Здесь 12345 — PID процесса, а 4 — номер дескриптора из вывода lsof. Это обнуляет файл, и место на диске сразу освобождается.

```shell
sudo truncate -s 0 /proc/12345/fd/4
```
Это часто спасает прод-серверы, когда диск внезапно заполняется логами, а перезапуск сервисов делать нельзя.
